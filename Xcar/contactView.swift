...
class contactView: UIViewController, UITextViewDelegate {
    ...
    func sendContactMessage() {
        ...
        if (userLanguageSelected == "ES") {
            supportTeam = "Soporte Xcar"
        }else {
            supportTeam = "Xcar Support"
        }
       ...
       /* getEmailQuery.observeSingleEvent(of: .value, with: {(snapshot) in
            ...
            if (snapshot.exists()) {
                ...
                sendEmail.sendEmail(usertoSend: "Xcar", recipient: "support@epicdevelopers.es", subject: self.subjectButton.title(for: UIControl.State.normal)!, message: "Usuario: " + self.plateName +  "<br>" + "Asunto: " + self.subjectButton.title(for: UIControl.State.normal)!+"<br>"+"Mensaje: " + self.messageField.text, fromName: self.plateName)
            }else{
                sendEmail.sendEmail(usertoSend: "Xcar", recipient: "support@epicdevelopers.es", subject: self.subjectButton.title(for: UIControl.State.normal)!, message: "Usuario: " + self.plateName +  "<br>" + "Asunto: " + self.subjectButton.title(for: UIControl.State.normal)!+"<br>"+"Mensaje: " + self.messageField.text, fromName: self.plateName)
